...
- Exact `specialist_only` versus `exact` mode is diagnostic; both preserve verified specialist bytes without notice. Repeated `coordinator_withheld` points to a separately bounded composition decision, not automatically to model escalation.
