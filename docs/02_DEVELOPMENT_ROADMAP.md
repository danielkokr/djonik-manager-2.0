...
### NOW — Wave B4: Structured task/checklist generation from studio intake (#26)

Turn rich studio intake (text, grouped messages, screenshots, PDFs) into a concise actionable task structure with useful substeps/checklist when warranted, while requiring explicit user intent before any verified Trello write. Actual Trello checklist-object mutation remains outside this bounded slice unless separately authorized after provider/tool discovery.
