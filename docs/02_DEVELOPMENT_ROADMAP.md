...
Add one bounded Claude-native `project-health` capability that turns fresh Trello evidence into a concise current-project PM interpretation: current work, waiting vs blocked, risk, staleness only where reliable activity evidence exists, and the next useful PM step. Project Health is an interpretation, never a stored competing source of truth. Historical reviews and proactive follow-up remain later Wave C slices.
