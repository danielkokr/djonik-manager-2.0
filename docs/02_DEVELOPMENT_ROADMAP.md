...
- **Wave C1 implementation and live acceptance are COMPLETE.** Only repository/GitHub administrative closeout remains: **#28 is ready to close after this closeout commit lands** (canonical NOW stays #28 in this local slice until the Product Owner reviews and commits these docs). No next issue is selected or promoted here; capability order in `docs/03` is not execution order;
