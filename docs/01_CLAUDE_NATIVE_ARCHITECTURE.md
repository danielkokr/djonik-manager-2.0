...
**Current accepted state — 2026-09-24 (#38 accepted, #33 NOW; [docs/50](50_ISSUE_38_CLOSEOUT_AND_33_PROMOTION.md)).** This supersedes the *#38 remains NOW / #33 not promoted* statements in the dated paragraphs below, which remain historical evidence. The accepted tuple entering #33:
- **Primary Djonik** `agent_01WGRHDBjQa3eMhoGJMmQ1dh` **v25**: Claude Sonnet 5, effort medium, speed standard.
- **Coordinator prompt:** `managed-agents/djonik.md`, byte-identical to v25 (3529 B, SHA-256 `3e204453…b4b6`).
- **Coordinator Skills:** accepted synchronized versions. task-management `skver_01CJQkVY1kp1urhBWQgHUYxW`, daily-planning `skver_01EL7d3fy4WWYBSsD3PmK9LQ`, weekly-planning `skver_01FdtnnkbePBjNJGTuJyAvhw` and studio-intake `skver_018sJv1GCnzfZRG4NbExbAzj` are each byte-identical to the repo. Production still references them as `latest`; explicit pinning belongs to #33.
- **Project Health specialist** `agent_01KNiQDzzPjaMU6LLF4mU6uM` **v4**, pinned in the roster.
- **Runtime** (application source): accepted #31 mutation verification, #32 completion/specialist provenance, #40 idempotent continuation, #36 work-history support, #37 project labels + Memory hygiene, and #38 voice / F3 PH composition.
...
**#33 host preparation — 2026-09-24 ([docs/54](54_ISSUE_33_ALWAYS_ON_HOST_AND_TELEGRAM_CUTOVER.md), pending Product Owner review).**
- **Host decision:** Hetzner Cloud CX23, Ubuntu 24.04 LTS, one systemd unit, outbound HTTPS only. The fallback is Fly.io with one Machine (stop-before-start); Railway is no longer the fallback.
- **Session continuity is unchanged:** one serving Session per process; a restart starts a new Session, and Memory is the durable layer.
- **Serving evidence:** `release:check -- <id> --serving` re-reads and attests the newest Telegram serving Session.
- **Not yet done:** the host is still not created. That is a Product Owner billing action.

**Production serving — current state 2026-09-24 (#33 accepted, [docs/55](55_ISSUE_33_HOSTED_TELEGRAM_FINAL_ACCEPTANCE.md)).** This supersedes the *not yet done / host not created* bullets above.
- **Hosting:** the Telegram adapter runs always-on on Hetzner Cloud CX23 (Ubuntu 24.04, Node 24) as one systemd unit, `djonik-telegram`. It serves app `a6a627d48353cc768a39e7a5a94c5f7c68154aa9`, release **r26**, Agent **v26** (specialist v4).
- **Session:** one process-scoped, pinned and attested Telegram serving Session at a time. A restart creates a new Session; Memory stays the durable layer across restarts.
- **Secrets:** host-side only (`/etc/djonik/djonik.env`, root 0600). The laptop is no longer part of serving.

**Production serving — #34 rollout 2026-09-24 ([docs/60](60_ISSUE_34_ACCEPTANCE_AND_39_PROMOTION.md)).** This supersedes the app revision in the paragraph above as current state: the host now serves app `d59a53261b813b6476b99173d693783e28e8586e`, which carries the accepted #34 commitment contract in the Memory resource instructions. Release r26, Agent v26, Skills, specialist v4 and the production Memory store are unchanged; `release:check -- r26 --serving` attested the new hosted Session.
...
**#38 source-only alignment, 2026-09-23:** [docs/40](40_DJONIK_INSTRUCTION_ARCHITECTURE_AUDIT.md) found that shared Trello field meanings lived mainly in the Project Health specialist, while coordinator planning/task Skills used the same fields without the same contract. The local coordinator source now proposes one concise global factual-semantics block; daily, weekly and task Skills use it without copying Project Health procedure. [docs/41](41_ISSUE_38_CONTRACT_DERIVED_RUBRIC.md) proposes a rubric derived from active instructions and accepted runtime boundaries. This paragraph supersedes the preceding **source/live byte-equality statement as a current claim**: the SHA above is the historical v24 read-back, while the edited repo source has not been applied to the production Agent. Model escalation is frozen; Sonnet 5 medium/standard remains the recorded production baseline, #38 remains NOW and unaccepted, and #33 is not promoted. No Agent, remote Skill, runtime, Memory or serving Session change is made by this source-only step.

**#38 remote synchronization, 2026-09-23:** After Product Lead review of the source-only alignment, the Product Owner authorized its remote sync. It supersedes the preceding paragraph's *not applied* statement as a current claim. New versions of three coordinator Skills were created from the committed repo files. Each was downloaded back and is byte-identical to its repo file:
- `task-management` `skill_01WS6JtY1GMu3rGZaKCVR9w1`: `skver_012fLb9Z…` → `skver_01CJQkVY1kp1urhBWQgHUYxW` (SHA-256 `e994d31a…c99c`);
- `daily-planning` `skill_01G9DtQEzPYxgw78riFh8k99`: `skver_01TDfMCn…` → `skver_01EL7d3fy4WWYBSsD3PmK9LQ` (`7fc9ff92…65f0`);
- `weekly-planning` `skill_01PTmbvLHJj1HuUxvDKhpaiE`: `skver_018soRHm…` → `skver_01FdtnnkbePBjNJGTuJyAvhw` (`0d6cca38…2737`).

`studio-intake` already matched the repo and stays `skver_018sJv1GCnzfZRG4NbExbAzj`. The previous remote `task-management` dated from 2026-09-18, before #37, so this sync also brings the accepted #37 project-label rules into the production Skill for the first time. Primary Agent v24 → **v25** changed only `system`, with a version-24 precondition. It now matches `managed-agents/djonik.md` byte for byte (3529 B; SHA-256 `3e204453…b4b6`). A read-back confirmed that every other field equals v24: Sonnet 5 / medium / standard, Skills still `latest`, tools, permissions, MCP, metadata, and the specialist v4 roster.

The Agent and its Sessions store these Skills as `latest`. Candidate Agents pin explicit `skver_` versions, so the only other `latest` reference is an archived validation coordinator. Whether an already-created `latest` Session re-resolves Skills later is not documented and was not tested live. No Telegram adapter process was running.

One isolated zero-inference Session, `sesn_015pyFLex524B9ASrk8fxHHP`, is prepared for the docs/41 smoke. It is pinned to v25 and uses the same environment and vault, with production Memory mounted `read_only` under the unchanged instructions. It has no events, zero tokens and zero list cost, and a 1¢ `max_list_cost` backstop. Paid validation has not run. #38 remains NOW and unaccepted, #33 is not promoted, and the Telegram serving path was not switched.
...
- Djonik is the primary user-facing coordinator; the current model baseline is Sonnet 5 medium (2026-09-23 Product Owner decision above).
