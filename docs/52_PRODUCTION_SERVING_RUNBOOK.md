...
Date: 2026-09-24. Status: **§4 Steps 1–2 executed** (Agent v26 created and attested; this revision serves r26) and the serving-boundary smoke passed on a validation Session — [docs/53](53_ISSUE_33_R26_CONTROLLED_CUTOVER.md). **Update 2026-09-24 — initial host setup, deploy and cutover are completed** ([docs/55](55_ISSUE_33_HOSTED_TELEGRAM_FINAL_ACCEPTANCE.md)): Hetzner CX23 serves `a6a627d` (r26 / Agent v26) with one systemd unit; the hosted Telegram smoke and a restart passed. §2 and §4 are now the historical record of that one-time setup. **This document is the routine operations (§3, §5, §7) and rollback (§6) runbook.** Every step marked **[AUTH]** is a live change that needs explicit Product Owner authorization; steps marked **[READ]** are read-only.
