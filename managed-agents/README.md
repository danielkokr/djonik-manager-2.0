...
- `project-health-specialist.md`: read-only Sonnet Project Health specialist (issue #28, Wave C1). **Exists as a live standalone agent** (`agent_01KNiQDzzPjaMU6LLF4mU6uM`); the live version is now **v2** (natural-PM-voice hardening, recorded in `claude-lock.json`). It is **not attached to the production Djonik coordinator**: production (`agent_01WGRHDBjQa3eMhoGJMmQ1dh`) has no roster. Only the temporary, non-production validation coordinator (`agent_01GFCLFYq6uLRHG8vMCrAgeK`, v3) pins specialist v2, for validation only. The v2 behavior has not been live-validated yet. A coordinator roster pins a specialist version, so any later specialist change also needs a separate roster update.
